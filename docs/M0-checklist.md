...
| 항목 | 상태 | 비고 |
|---|---|---|
| 공공데이터포털 키 | 🟡 **설정됨 · 호출 실패** | `.env`에 들어갔으나 403. 아래 참조 |
| 사람인 API | ⬜ 승인 대기 | 승인 후 `.env`에 키만 넣으면 됨. M7까지 되면 된다 |
| 카카오 앱 | ⬜ 대기 | M9에서 필요 |
| 도메인 · DNS | ⬜ 대기 | M11에서 필요 |
...
키는 `.env`에 설정됐고 `check-keys`도 인식한다. 그러나 실제 호출이 실패한다:
...
**인코딩/디코딩 혼동이 아니다.** 키를 원문 그대로 붙인 경우와 URL 디코딩 후 재인코딩한
경우 **둘 다 같은 에러**였다. 따라서 남은 원인은 둘 중 하나다:

1. **해당 API를 활용신청하지 않았다** ← 가능성 높음
   일반 인증키는 계정당 하나지만, **API마다 따로 활용신청**해야 한다.
   마이페이지 → 데이터활용 → Open API → **활용신청 현황**에서 목록을 확인할 것
2. **발급/신청 직후라 아직 반영 전** (보통 1시간, 드물게 최대 1일)

**M3 착수 전까지 해결하면 된다.** M1·M2는 이 키 없이 진행된다.

해결 후 검증:

```bash
uv run python -m app.cli check-keys    # 설정 여부
# 실제 호출까지 확인하려면 활용신청한 API의 상세 페이지에서 제공하는
# 샘플 요청 URL을 그대로 curl로 때려본다
```
