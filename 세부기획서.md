...
2. `tests/fixtures/<site>/list.html` 스냅샷 저장 + 파서 테스트 1개
3. 웹 UI에서 소스 row 1개 추가
...
| robots.txt | 소스 등록 시 1회 확인하고 결과를 `sources.config`에 기록. 차단 경로는 크롤링하지 않는다 |
...
|---|---|---|---|
| **공공데이터포털** (워크넷·잡알리오) | 공식 오픈 API. 인증키 **자동 승인**, 일 1,000건 | **1** | 낮음. 첫 크롤러로 채택 |
| **링커리어** `linkareer.com/list/activity` | **SSR HTML 확인.** 목록 HTML에 제목·기업·마감일(D-n)·상세링크가 직접 포함 | **3** | 마크업 변경. → 골든 파일 테스트로 방어. **Playwright 불필요** |
| **인디스워크** `inthiswork.com` | 외부 요청에 **HTTP 403** 반환 | **미확정** | 봇 차단 또는 WAF. M7 착수 시 UA/Accept 헤더 조정, `/feed`·`/wp-json` 우선 확인. 그래도 막히면 후순위 |
| **금융투자협회** 채용공고 | 게시판형. 정적 HTML 가능성 높음 | **3 (추정)** | 목록 페이징 파라미터 확인 필요 |
...
- [ ] `uv run uvicorn app.main:app` 기동 후 `GET /healthz`가 200을 반환한다
- [ ] `.env` 없이 실행하면 어떤 변수가 빠졌는지 명시된 에러가 뜬다
- [ ] `uv run ruff check .`와 `uv run pytest`가 통과한다
- [ ] GitHub Actions가 초록불이다
...
- [ ] `alembic upgrade head` → 테이블 10종 + 인덱스가 생성된다
- [ ] `alembic downgrade base` → 깨끗이 롤백된다
- [ ] 시드 실행 후 `keywords` 8건이 조회된다
- [ ] 모델 제약(UNIQUE) 위반 시 `IntegrityError`가 나는 테스트가 통과한다
...
- `datagokr_worknet.py` — **티어 1 (공식 API)**
- `linkareer.py` — **티어 3 (HTTP + HTML)**
- 두 사이트의 골든 픽스처 + 파서 테스트
...
- [ ] `crawl datagokr-worknet`과 `crawl linkareer`가 모두 `list[RawJob]`을 반환한다
- [ ] 두 크롤러의 반환값이 동일한 필드 계약을 만족한다 (타입 체크 + 테스트로 확인)
- [ ] 골든 파일 테스트가 네트워크 없이 통과한다
- [ ] `parse()`가 DB·네트워크를 호출하지 않음이 코드 리뷰로 확인된다
- [ ] `HttpClient`의 429 재시도, 5MB 상한, 304 스킵이 각각 테스트로 검증된다
...
- [ ] 같은 소스를 연속 2회 실행 → 2회차 `items_new = 0`
- [ ] 픽스처의 마감일을 바꿔 재실행 → `job_posting_revisions` 1건, `changed_fields`에 old/new
- [ ] 조회수만 바뀐 픽스처로 재실행 → revision **0건** (해시 제외 필드 검증)
- [ ] 제목 앞에 `[신입]` 태그가 붙은 픽스처 → 같은 공고로 인식 (normalize 검증)
- [ ] 목록에서 3회 사라진 공고가 `is_open=false`, `closed_at` 세팅됨
...
- [ ] `데이터` 등록 시 "데이터 분석가" 공고가 매칭된다
- [ ] exclude `경력 5년` 등록 시 해당 공고가 매칭에서 제외된다
- [ ] 매칭 결과에 `matched_snippet`이 저장돼 근거를 볼 수 있다
- [ ] `EXPLAIN ANALYZE`에서 title 검색이 GIN 인덱스를 탄다
- [ ] 가중치 순 정렬 시 `데이터분석` 매칭이 `AI` 매칭보다 위에 온다
...
- [ ] 워커 기동 후 개입 없이 3사이클 이상 자동 수집된다
- [ ] 수집 중 같은 소스를 수동 트리거 → `skipped` 기록, 중복 실행 없음
- [ ] 소스를 `is_active=false` 하면 다음 사이클부터 실행되지 않는다
- [ ] `crawl_runs`에 소요시간·건수·HTTP 상태 요약이 남는다
- [ ] 워커 강제 종료 후 재시작 → advisory lock 잔류 없이 정상 동작
...
- `saramin_api.py` — 사람인 API (M0 승인 완료 전제)
- 잡플래닛은 ToS 확인 결과에 따라 결정 (미확인 시 미구현)
- `docs/adding-a-source.md` — 사이트 추가 가이드
- `docs/sources/<slug>.md` — 사이트별 조사 기록

**DoD**

- [ ] 최소 4개 소스가 동시에 스케줄러에서 돈다
- [ ] **크롤러 추가 커밋의 diff가 신규 파일 + 픽스처 + 설정에만 한정된다** — `collector.py`, `normalizer.py`, `deduplicator.py`, `keyword_matcher.py` 무변경 (NFR-05)
- [ ] 각 소스의 robots.txt 확인 결과가 문서에 기록돼 있다
- [ ] 인디스워크 403이 해결됐거나, 해결 불가 사유와 대안이 문서화됐다
...
- [ ] 비로그인 상태에서 `/admin/*` 접근 시 로그인으로 리다이렉트된다
- [ ] `데이터` 검색이 1초 이내에 응답한다
- [ ] 소스를 UI에서 추가하면 다음 스케줄 사이클에 반영된다
- [ ] 실패한 크롤 실행의 원인이 대시보드에서 확인된다
- [ ] 워커를 정지시키면 15분 내 UI에 경고 배너가 뜬다
...
- [ ] 새 매칭 공고 발생 → 브라우저에 Web Push 알림이 도착하고, 클릭 시 공고 상세로 이동한다
- [ ] 같은 상황에서 카카오톡 "나와의 채팅"에 메시지가 도착한다
- [ ] 워커를 2회 연속 실행해도 **같은 공고 알림이 두 번 오지 않는다**
- [ ] 방해금지 시간대에 발생한 알림이 `scheduled_at`으로 큐잉되고, 시간대가 지나면 발송된다
- [ ] 사이트 5개에서 매칭이 나와도 알림은 **묶음 1건**이다
- [ ] 카카오 refresh token을 의도적으로 무효화 → Web Push로 재인증 알림 + UI 배너가 뜬다
...
- [ ] 소스 URL을 잘못된 값으로 바꿔 5회 실패 → 자동 비활성 + 알림 1회
- [ ] 픽스처를 빈 HTML로 바꿔 수집 → 급감 감지 알림
- [ ] 타임아웃·429·파싱 오류가 각각 다른 `error_type`으로 기록된다
- [ ] `pytest --cov` 서비스 계층 80% 이상
- [ ] 워커 1사이클 피크 메모리가 **150MB 이하**로 측정된다 (NFR-01)
- [ ] DB를 중단시켰다가 재기동 → `pool_pre_ping`으로 자동 복구된다
