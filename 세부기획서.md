...
| FR-08 | 키워드 매칭 | 활성 include가 있을 때 직무를 특정하는 include 매칭 && exclude 미매칭인 공고만 저장·표시. 매칭 근거 저장 | MUST | M5 | 매칭된 공고마다 어떤 키워드가 어느 필드에서 걸렸는지 조회된다 |
...
마감일을 제공하는 공고는 KST 기준 마감일의 다음 날이 끝난 뒤 보존 정책에서 행 전체와 연관
매칭·알림·변경 이력을 삭제한다. 마감일을 제공하지 않는 소스는 시점을 추측하지 않고 위의
3회 미노출 판정을 유지한다. `신입`, `인턴`은 직무가 아닌 보조 조건이므로, 이 키워드만 일치한
공고는 저장하지 않는다.
...
| **링커리어** `linkareer.com/list/recruit` | **SSR HTML 확인.** `activityTypeID=5`, `OPEN`으로 채용공고만 선택하고 제목·마감일·상세링크를 얻음 | **3** | 활동 목록은 사용하지 않음. 골든 파일 테스트로 방어. **Playwright 불필요** |
| **인디스워크** `inthiswork.com/entry` | 사용자용 채용 화면, WordPress 채용 분류 JSON 확인 | **2** | 403 재현 안 됨. 신입/인턴·주니어경력 분류만 저빈도로 조회 |
...
**보존 정책 잡** (매일 00:05 KST, `services/retention.py`)

| 대상 | 정책 |
|---|---|
| 마감일 다음 날이 지난 공고 | KST 기준 다음 날 24:00 뒤 행과 연관 매칭·알림·변경 이력을 삭제 |
...
- [x] GitHub Actions가 초록불이다 — CI 실행 `32621411424`에서 PostgreSQL 마이그레이션·lint·format·132개 테스트가 모두 통과했다
...
- [x] `https://<도메인>`에 유효한 인증서로 접속된다
- [x] `systemctl status`에서 두 유닛이 모두 active다
- [x] **VM 강제 재부팅 후 5분 내 두 유닛이 자동 기동하고 수집이 재개된다** (NFR-04)
- [x] `free -h` 기준 사용 메모리가 750MB 이하다 (NFR-01)
- [x] 백업 파일이 생성되고, **덤프를 별도 DB에 복구해 공고 건수가 일치함을 확인**했다
- [x] 외부에서 5432 포트가 닫혀 있다 (`nmap` 확인)
- [x] Web Push와 카카오톡 알림이 운영 환경에서 실제로 도착한다

> 구현 중 확인: 개발 PC에 `nmap`이 없어 외부 TCP 연결 검사(`Test-NetConnection`)로
> 5432 포트의 차단을 검증했다. Azure NSG와 UFW에는 22·80·443만 허용했다.
...
- [x] 30일간 수동 개입 없이 운영됐다 (또는 개입 사유가 모두 기록됐다) — 개인 단독 사용 범위로 사용자 요청에 따라 장기 관측을 완료 조건에서 제외했다 (2026-08-23)
- [x] 메모리 사용량이 30일 내내 예산 안에 있었다 — 개인 단독 사용 범위로 사용자 요청에 따라 장기 관측을 완료 조건에서 제외했다 (2026-08-23)
- [x] README만 읽고 처음 보는 사람이 아키텍처와 데이터 흐름을 설명할 수 있다
- [x] 모든 주요 기술 선택에 대해 "왜 이걸 골랐는가"를 문서로 답할 수 있다

> M12 구현 변경(2026-08-23): 채용공고 자동 알림은 `Asia/Seoul` 기준 09:00 cron의
> 하루 한 번 요약으로 바꿨다. 관리자는 수동 즉시 발송을 할 수 있고, `app_settings`의 전역
> 활성화 설정으로 카카오·Web Push 채용공고 알림을 함께 켜고 끈다. 30일 운영 관측은
> `docs/operations.md`에 시작했다.
...
- [링커리어 채용 목록](https://linkareer.com/list/recruit?filterBy_activityTypeID=5&filterBy_status=OPEN&orderBy_direction=DESC&orderBy_field=RECENT&page=1)
- [인디스워크 채용](https://inthiswork.com/entry)
