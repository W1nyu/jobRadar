...
활성 관심 키워드가 설정된 운영 상태에서는 `데이터`, `AI`, `개발`처럼 직무를 특정하는 키워드가
일치한 공고만 저장한다. `신입`, `인턴`은 보조 조건이므로 직무 관심 키워드와 함께 일치할 때만
저장한다. 마감일이 있는 공고는 KST 기준 마감일의 다음 날이 끝난 뒤 DB와 관리 화면에서 함께
제거한다.
